...
def processArticle(article_id, article, df, tokenizer, new_df, save_processed_text=False, debug=False):
    ...
    all_bop_index, bop_to_eop, bop_to_label, bop_to_start_end, bop_to_label_CE= tokenizeAndGetIndex(text, processed_dict, tokenized_new_text, max_num_bop)
    if debug:
        print("---------------------")
        print(all_bop_index)
        print("---------------------")
    ...
    while start_index < all_bop_index[-1]:
        ...
        bop_index_in_window_local, eop_index_in_window_local, labels, labels_CE, labels_in_context, labels_mask, bop_index_in_window, chunk_in_tokens, bop_to_start_in_context, bop_to_end_in_context, context_len_for_prediction, min_two_sides = truncateAndCreateDatapoints_new(tokenized_new_text, all_bop_index, bop_to_eop, bop_to_label, bop_to_label_CE, start_index, end_index, bop_to_start_end)
        ...
        new_df = new_df.append({'bop_index' : bop_index_in_window_local, 'eop_index' : eop_index_in_window_local, 'labels' : labels, 'labels_CE' : labels_CE, f'{context_length}_chunk_tokens' : padded_chunk_in_tokens, 
            ...
            'start_in_context': bop_to_start_in_context, 'end_in_context' : bop_to_end_in_context, 'context_len_for_prediction': context_len_for_prediction, 'min_two_sides': min_two_sides}, ignore_index = True)

        start_index += step_size
    return max_num_bop, new_df, bop_to_start_end, set(bop_index_processed)
def truncateAndCreateDatapoints_new(tokenized_new_text, all_bop_index, bop_to_eop, bop_to_label, bop_to_label_CE, window_start, window_end, bop_to_start_end, debug=False):
    ...
    for bop_index in bop_index_in_window: # bop_index_in_window all old 
        ...
        labels_CE.append(bop_to_label_CE[bop_index])
    ...
    return bop_index_in_window_local, eop_index_in_window_local, labels, labels_CE, labels_in_context, labels_mask, bop_index_in_window, chunk_in_tokens, bop_to_start_in_context, bop_to_end_in_context, context_len_for_prediction, min_two_sides
...
def tokenizeAndGetIndex(text, processed_dict, tokenized_new_text, max_num_bop):
    ...
    for i in range(0, max_num_bop + 1):
        ...
        for j in range(len(bop_index_list)):
            ...
            bop_to_label_CE[bop_index_list[j]] = processed_row[j]['one_label_for_CE']
    ...
    return all_bop_index, bop_to_eop, bop_to_label, bop_to_start_end, bop_to_label_CE
...
df['one_label_for_CE'] = df.apply(lambda x: max(x['techniques']), axis=1)
...
max_num_bop = 0
tokenizer = RobertaTokenizer.from_pretrained("roberta-base", additional_special_tokens=("<bop0>", "<eop0>", "<bop1>", "<eop1>", "<bop2>", "<eop2>", "<bop3>", "<eop3>"))
...
if split: 
    pickle.dump(new_df, open(f'../split_data/processed_data/{curr_split}/{train_or_dev}_{context_length}_chunk_CE.pkl', "wb"))
    new_df.to_csv(f'../split_data/processed_data/{curr_split}/{train_or_dev}_{context_length}_chunk_CE.csv', index = False)
else:
    pickle.dump(new_df, open(f'../processed_data/{train_or_dev}_{context_length}_chunk_CE.pkl', "wb"))	
    new_df.to_csv(f'../processed_data/{train_or_dev}_{context_length}_chunk_CE.csv', index = False)
    print(f'../processed_data/{train_or_dev}_{context_length}_chunk_CE.csv')
