...
def truncateAndCreateDatapoints_new(tokenized_new_text, all_bop_index, bop_to_eop, bop_to_label, window_start, window_end, bop_to_start_end, debug=False):
    ...
    return bop_index_in_window_local, eop_index_in_window_local, labels, labels_in_context, labels_mask, bop_index_in_window, chunk_in_tokens, bop_to_start_in_context, bop_to_end_in_context, context_len_for_prediction, min_two_sides
...
for article_id in [258]:
    article = df.loc[df['article_id'] == article_id]
    curr_max_num_bop, new_df, bop_to_start_end, bop_processed = processArticle(article_id, article, df, tokenizer, new_df, save_processed_text=True)
...
# if split: 
#     pickle.dump(new_df, open(f'../split_data/processed_data/{curr_split}/{train_or_dev}_{context_length}_chunk.pkl', "wb"))
#     new_df.to_csv(f'../split_data/processed_data/{curr_split}/{train_or_dev}_{context_length}_chunk.csv', index = False)
# else:
#     pickle.dump(new_df, open(f'../processed_data/{train_or_dev}_{context_length}_chunk.pkl', "wb"))	
