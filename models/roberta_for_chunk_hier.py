...
def train(model_name, context_length, batch_size, num_epochs, data_set, output_dir, model_num, aux_weight_train, aux_weight_eval, gold_file, dropout=0, num_labels=14, learning_rate=5e-5, adam_beta1=0.9, adam_beta2=0.999, adam_epsilon=1e-8, weight_decay=1e-5, random_seed=10, eval_frequency=25):
    def evaluate_multiple(best_f1, weight):
        valid_loss = model_eval.predict_and_write_to_file_for_chunk_hier(context_length, model, valid_loader, f"outputs/baseline-output-TC-{model_num}-{epoch}-{weight}.txt", weight)
        print(f"outputs/baseline-output-TC-{model_num}-{epoch}-{weight}.txt")
        f1, precision, recall, f1_per_class, _, _ = model_eval.score(f"outputs/baseline-output-TC-{model_num}-{epoch}-{weight}.txt", gold_file)
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), f'outputs/best_model_hier-{random_seed}-{model_num}-{context_length}-{weight}.pt') 

            with open(f"outputs/baseline-output-TC-{model_num}-{epoch}-{weight}.txt",'r') as firstfile, open(f'outputs/pred_hier-{random_seed}-{model_num}-{context_length}-{weight}.txt','w') as secondfile:
        ...
        with torch.autograd.set_detect_anomaly(True):
            for batch_index, batch in enumerate(train_loader):
                ...
                if num_batches % eval_frequency == 0:
                    ...
                    #     best_f1_list[i], valid_loss = evaluate_multiple(best_f1_list[i], i / 10)
                    best_f1_0, valid_loss = evaluate_multiple(best_f1_0, 0)
                    best_f1_same, _ = evaluate_multiple(best_f1_same, aux_weight_eval)
                    best_f1_1, _ = evaluate_multiple(best_f1_1, 1)
    ...
    test_loader = DataLoader(data_set['test'], batch_size=batch_size, collate_fn=collate_fn)
    for weight in [0, 0.5, 1]:
        model.load_state_dict(torch.load(f'outputs/best_model_hier-{random_seed}-{model_num}-{context_length}-{weight}.pt'))
        model_eval.predict_and_write_to_file_for_chunk_hier(context_length, model, test_loader, f"outputs/test_output_hier-{model_num}-{weight}.txt", weight)
