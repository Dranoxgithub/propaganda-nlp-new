...
def train(model_name, context_length, batch_size, num_epochs, data_set, output_dir, model_num, gold_file, dropout=0, num_labels=14, learning_rate=5e-5, adam_beta1=0.9, adam_beta2=0.999, adam_epsilon=1e-8, weight_decay=1e-5, random_seed=10, eval_frequency=25):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for batch_index, batch in enumerate(train_loader):
                ...
                if f1 > best_f1:
                    best_f1 = f1
                    torch.save(model.state_dict(), f'outputs/best_model_chunk-{random_seed}-{model_num}-{context_length}.pt') 

                    with open(f"outputs/baseline-output-TC-{model_num}-{epoch}.txt",'r') as firstfile, open(f'outputs/pred_chunk-{random_seed}-{model_num}-{context_length}.txt','w') as secondfile:
    ...
    model_eval.predict_and_write_to_file_for_chunk(context_length, model, test_loader, f"outputs/test_output_chunk-{model_num}.txt")
