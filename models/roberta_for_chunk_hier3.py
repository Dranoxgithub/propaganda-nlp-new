...
class BERTClass(torch.nn.Module):
    def __init__(self, model_name, num_labels, dropout):
        ...
        self.linear_relu_stack = torch.nn.Sequential(
            torch.nn.Linear(4 * 100, 1024),
            torch.nn.ReLU(),
            torch.nn.Linear(1024, 1024),
        )
        self.embedding = torch.nn.Parameter(torch.empty(20, 100))
        torch.nn.init.uniform_(self.embedding, -0.1, 0.1)

    def forward(self, ids, mask):
        padding = torch.empty(100).to(device)
